...
export default function ProfileCard({user}:{user:User}) {
  return (
    <div className="card" style={{display:'flex', gap:'1.25rem'}}>
      <div>
        <img className="avatar" src={user.avatarUrl || `https://api.dicebear.com/8.x/initials/svg?seed=${user.firstName}%20${user.lastName}`} alt="avatar" />
      </div>
      <div style={{flex:1}}>
        <h2 style={{margin:'0 0 .25rem'}}>{user.firstName} {user.lastName}</h2>
